...
function AdmissionsPage({ navigate, isMobile }) {
  ...
  return (
      <div style={{ background: C.lightGray, minHeight: "100vh" }}>
        ...
        {/* Hero */}
        <div style={{ background: `linear-gradient(135deg,${C.navy},${C.bright})`, padding: isMobile ? "28px 16px" : "44px 24px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
              ...
              <h1 style={{ color: C.white, fontSize: isMobile ? 22 : "clamp(22px,4vw,38px)", fontWeight: 800, fontFamily: "'Playfair Display',Georgia,serif", margin: 0 }}>
                Qabul 2026-2027
              </h1>
            </div>
            <p style={{ color: "rgba(255,255,255,0.7)", fontSize: 14, marginBottom: 22 }}>
              O'zbekiston Milliy universitetiga qabul jarayoni haqida to'liq ma'lumot
            </p>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: isMobile ? "16px 12px 80px" : "32px 24px 60px", boxSizing: "border-box" }}>
          ...
          {/* Steps */}
          <div style={{ background: C.white, borderRadius: 16, padding: isMobile ? "16px" : "28px", marginBottom: 24, border: `1px solid ${C.gray}` }}>
            ...
            <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: isMobile ? 12 : 0, alignItems: isMobile ? "stretch" : "flex-start", position: "relative" }}>
              ...
              {admissionSteps.map((step) => (
                  <div key={step.n} style={{ flex: 1, display: "flex", flexDirection: isMobile ? "row" : "column", alignItems: isMobile ? "flex-start" : "center", gap: isMobile ? 12 : 10, textAlign: isMobile ? "left" : "center", position: "relative", zIndex: 1, minWidth: 0 }}>
                    <div style={{ width: isMobile ? 44 : 62, height: isMobile ? 44 : 62, borderRadius: "50%", background: `linear-gradient(135deg,${C.bright},${C.navy})`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: `0 4px 14px rgba(40,69,214,0.28)`, border: `3px solid ${C.white}` }}>
                      <Ico name={step.icon} size={isMobile ? 20 : 26} color={C.white} sw={1.5}/>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: isMobile ? 13 : 13, fontWeight: 700, color: C.dark, marginBottom: 4 }}>{step.title}</div>
                ...
                <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", minWidth: isMobile ? 520 : "auto" }}>
                    ...
                    <tr style={{ background: C.lightGray }}>
                      {["Yo'nalish", "Davomiyligi", "Til", "Grant", "Kontrakt", "Ball"].map(h => (
                          <th key={h} style={{ padding: "10px 14px", fontSize: 11, fontWeight: 700, color: C.mid, textAlign: "left", textTransform: "uppercase", letterSpacing: "0.5px", borderBottom: `1px solid ${C.gray}`, whiteSpace: "nowrap" }}>{h}</th>
                      ))}
                    </tr>
                    </thead>
                    <tbody>
                ...
                {faqs.map((faq, i) => (
                    <div key={i} style={{ borderBottom: i < faqs.length - 1 ? `1px solid ${C.lightGray}` : "none" }}>
                      <div onClick={() => setOpenFaq(openFaq === i ? null : i)}
                           style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", cursor: "pointer", transition: "background 0.15s" }}
                      ...
                      {openFaq === i && (
                          <div style={{ padding: "14px 18px 14px", fontSize: 14, color: C.mid, lineHeight: 1, animation: "fadeUp 0.2s ease" }}>{faq.a}</div>
                      )}
                    </div>
                ))}
              </div>
                ...
                <aside style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                  <div style={{ background: C.white, borderRadius: 14, padding: 18, border: `1px solid ${C.gray}` }}>
                    ...
                    {deadlines.map((d, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 13, paddingBottom: 13, borderBottom: i < deadlines.length - 1 ? `1px solid ${C.lightGray}` : "none" }}>
                          ...
                          <div>
                            <div style={{ fontSize: 12, color: C.mid, marginBottom: 2 }}>{d.l}</div>
                            <div style={{ fontSize: 13, fontWeight: 700, color: d.color }}>{d.d}</div>
                          </div>
                        </div>
                    ))}
                  </div>
                  <div style={{ background: `linear-gradient(135deg,${C.orange},#ff9f6a)`, borderRadius: 14, padding: 20 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                      ...
                      <h3 style={{ fontSize: 15, fontWeight: 700, color: C.white, margin: 0 }}>Hujjat topshirish</h3>
                    </div>
