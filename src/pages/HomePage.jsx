  ...
  return (
      <div
          ...
          style={{
            ...
            borderRadius: 12,
            overflow: "hidden",
            boxShadow: hov ? "0 16px 40px rgba(13,26,99,0.14)" : "0 4px 16px rgba(13,26,99,0.07)",
      ...
      >
        {compact ? (
            <>
              <div style={{ width: 56, minWidth: 56, height: 56, background: `linear-gradient(135deg,${item.accent}18,${item.accent}30)`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, borderRadius: 10, margin: 8 }}>
                <Ico name={item.icon} size={24} color={item.accent} sw={1.5}/>
              </div>
              <div style={{ padding: "10px 12px 10px 0", flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, flexWrap: "wrap" }}>
                  <Badge label={item.cat} color={item.accent}/>
                  <span style={{ color: C.light, fontSize: 11, flexShrink: 0 }}>{item.date}</span>
                  <span style={{ color: C.light, fontSize: 11, flexShrink: 0, display: "flex", alignItems: "center", gap: 3 }}>
                <Ico name="clock" size={10} color={C.light}/>{item.time || "09:00"}
              </span>
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: C.dark, lineHeight: 1.35, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", wordBreak: "break-word" }}>
                  {item.title}
                </div>
              </div>
            </>
            ...
            <>
              ...
              <div style={{ padding: 20 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                  <Badge label={item.cat} color={item.accent}/>
                  <span style={{ color: C.light, fontSize: 12 }}>{item.date}</span>
                  <span style={{ color: C.light, fontSize: 11, display: "flex", alignItems: "center", gap: 3 }}>
                <Ico name="clock" size={10} color={C.light}/>{item.time || "09:00"}
              </span>
                </div>
                <h3 style={{ fontSize: 15, fontWeight: 700, color: C.dark, lineHeight: 1.4, marginBottom: 8, fontFamily: "'Playfair Display',Georgia,serif", wordBreak: "break-word" }}>{item.title}</h3>
                <p style={{ fontSize: 13, color: C.mid, lineHeight: 1.6, margin: 0, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical" }}>{item.desc}</p>
...
function HomePage({ navigate, isMobile }) {
  ...
  const stats = [
    ...
    { v: 8400, l: "Talabalar",            s: "", icon: "users"      },
  ...
  return (
      <div style={{ overflowX: "hidden" }}>
        ...
        {/* HERO */}
        <section style={{ background: `linear-gradient(135deg,${C.navy} 0%,${C.blue} 55%,${C.bright} 100%)`, padding: isMobile ? "44px 16px 60px" : "80px 24px 100px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative" }}>
            <div style={{ maxWidth: 700 }}>
              <h1 style={{ color: C.white, fontSize: isMobile ? "26px" : "clamp(30px,5vw,52px)", fontWeight: 800, lineHeight: 1.15, fontFamily: "'Playfair Display',Georgia,serif", marginBottom: 16, animation: "fadeUp 0.6s 0.1s ease both" }}>
                Nukus davlat<br/>
                <span style={{ background: `linear-gradient(90deg,${C.orange},#ffb88a,${C.orange})`, backgroundSize: "200% auto", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", animation: "shimmer 3s linear infinite" }}>
              ...
              <div style={{ maxWidth: 1200, margin: "0 auto" }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)" }}>
                  {stats.map((s, i) => {
                    ...
                    return (
                        <div key={i} style={{ textAlign: "center", padding: "28px 16px", borderRight: i < 3 ? `1px solid rgba(255,255,255,0.1)` : "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                          ...
                          <div style={{ fontSize: "clamp(26px,5vw,44px)", fontWeight: 800, color: C.orange, fontFamily: "'Playfair Display',Georgia,serif", lineHeight: 1 }}>{n.toLocaleString()}{s.s}</div>
                          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.65)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "1.2px" }}>{s.l}</div>
                        </div>
                    );
                  })}
                </div>
        ...
        {/* QUICK LINKS */}
        <section style={{ maxWidth: 1200, margin: "0 auto", padding: isMobile ? "32px 12px 20px" : "56px 24px 36px", boxSizing: "border-box" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(3,1fr)" : "repeat(6,1fr)", gap: isMobile ? 8 : 10 }}>
            {ql.map((lk, i) => (
                <div key={lk.label} onClick={() => navigate(lk.page)}
                     style={{ background: C.white, borderRadius: 14, padding: isMobile ? "12px 6px" : "20px 14px", textAlign: "center", cursor: "pointer", boxShadow: `0 2px 12px rgba(13,26,99,0.06)`, border: `1px solid ${C.gray}`, animation: `fadeUp 0.4s ${i*55}ms ease both`, transition: "all 0.25s ease" }}
                ...
                >
                  <div style={{ width: isMobile ? 36 : 48, height: isMobile ? 36 : 48, borderRadius: 12, background: `linear-gradient(135deg,${C.bright}12,${C.orange}10)`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 8px" }}>
                    <Ico name={lk.icon} size={isMobile ? 18 : 24} color={C.bright} sw={1.6}/>
                  </div>
                  <div style={{ fontWeight: 700, fontSize: isMobile ? 10 : 12, color: C.dark, lineHeight: 1.3, wordBreak: "break-word" }}>{lk.label}</div>
        ...
        {/* NEWS */}
        <section style={{ maxWidth: 1200, margin: "0 auto", padding: isMobile ? "0 12px 80px" : "0 24px 60px", boxSizing: "border-box" }}>
          <div style={{ display: "flex", alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between", marginBottom: 20, flexDirection: isMobile ? "column" : "row", gap: 10 }}>
            <div>
              ...
              <h2 style={{ fontSize: "clamp(18px,3vw,28px)", fontWeight: 800, color: C.dark, fontFamily: "'Playfair Display',Georgia,serif", margin: 0 }}>Universitetdagi voqealar</h2>
            </div>
